...
class TopIOTransform extends Transform {
  ...
  def execute(state: CircuitState): CircuitState = {
    ...
    val ioPair: Seq[(ReferenceTarget, ReferenceTarget, String)] = {
      /** get innerIOs. */
      ...
      innerIOs.map { case (t, n) =>
        (t, topIOMap(n), instanceGraph.findInstancesInHierarchy(t.moduleTarget.module).head.foldLeft("_TOPIO")(_ + "_" + _.name))
      }
    }
    val bottomModels: Seq[ModuleTarget] = ioPair.map(_._1.moduleTarget)
    /** collect port information from entire circuit. */
    val ioInfo: Map[ReferenceTarget, (Type, Direction)] = state.circuit.modules.flatMap { m =>
      m.ports.map {
        p: Port => Target.asTarget(ModuleTarget(state.circuit.main, m.name))(WRef(p)) -> (p.tpe, p.direction)
      }
    }.toMap
